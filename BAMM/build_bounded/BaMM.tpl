...
PARAMETER_SECTION
  ...
  // Bounded variant: log_Pmax and log_I_halfSat are bounded to physical ranges
  // so that weakly-identified streams (flat-diel light response, e.g. CC, cow)
  // cannot run off to ~1e23 along the Pmax/I_halfSat ridge. I_halfSat is capped
  // at exp(8.517) ~= 5000 (just above the max ~2840 seen across identified
  // streams), which keeps it in the linear-response regime so GPP (the
  // Pmax/I_halfSat ratio x light) is preserved; Pmax is left generous so it is
  // pinned by the identified slope rather than by its own bound.
  init_bounded_number log_Pmax(-6.9,11.5,Light_sat_Pmax_phase);
  init_bounded_number log_I_halfSat(-6.9,8.517,Light_sat_I_halfSat_phase);
